...
./alpine -r . -b /dev -b /sys -b /proc -b /tmp \
  ...
  wget -O  windows10.qcow2 https://cdn.bosd.io.vn/tiny10-x64.qcow2 && cd /home/container && \
...
run_system() {
  ...
  $DOCKER_RUN "qemu-system-x86_64 -device qemu-xhci -device usb-tablet -device usb-kbd -cpu EPYC-Milan,+sse,+sse2,+sse4.1,+sse4.2,hv-relaxed -smp sockets=1,cores=$(nproc --all),threads=1 -overcommit mem-lock=off -m "8192",slots=2 -drive file=windows10.qcow2,aio=native,cache.direct=on,if=virtio -device virtio-gpu-pci,xres=1366,yres=768 -device virtio-net-pci,netdev=n0 -netdev user,id=n0,hostfwd=tcp::"25414"-:8080 -boot c -drive file=OVMF.fd,format=raw,readonly=on,if=pflash -device virtio-balloon-pci -device virtio-serial-pci -device virtio-rng-pci -display vnc=0.0.0.1:1,password=on -monitor stdio < qemu_cmd.txt"                         
