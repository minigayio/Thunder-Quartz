...
./alpine -r . -b /dev -b /sys -b /proc -b /tmp \
  --kill-on-exit -w /home/container /bin/sh -c "apk update && apk add bash xorg-server git virtiofsd python3 virtiofsd py3-pip py3-numpy \
    xinit xvfb fakeroot qemu qemu-img qemu-system-x86_64 qemu-kvm \
  virtualgl mesa-dri-gallium mesa-dri-drivers libgl1-mesa-dri libegl-mesa0 mesa-utils \
...
run_system() {
  ...
  $DOCKER_RUN "qemu-system-x86_64 -m "4096" -smp $(nproc --all) -cpu EPYC-Milan -enable-kvm -nic user,hostfwd=tcp::"25414"-:3389 -drive file=windows11.qcow2 -drive file=fat:rw:shared -usbdevice tablet -device virtio-vga-gl -display vnc=127.0.0.1:1,gl=on,password=on -monitor stdio < qemu_cmd.txt"                         
